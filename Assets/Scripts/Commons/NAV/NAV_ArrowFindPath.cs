...
public class NAV_ArrowFindPath : MonoBehaviour,IInitialization
{
    ...
    /// 开启导航
    /// </summary>
    /// <param name="moveEndPoint">移动的目标点</param>
    /// <param name="lineGameObject">画线组件</param>
    /// <returns></returns>
    public IEnumerator FindPath(Transform moveEndPoint)
    {
        while (true)
        {
            ...
            for (int i = 1; i <= 4; i++)
            {
                lineGameObject.SetPosition(lineGameObject.positionCount - i,
                    lineGameObject.GetPosition(lineGameObject.positionCount - 4));
                arrowList[lineGameObject.positionCount - i] =
                    lineGameObject.GetPosition(lineGameObject.positionCount - 4);
            }

            lineGameObject.material.SetTextureScale("_MainTex",
                new Vector2(UVTiling(arrowList.ToArray(), lineGameObject.startWidth), 1f));

            arrowList.Clear();

            // 移动效果
            lineGameObject.material.SetTextureOffset("_MainTex",
                new Vector2(UVOffset(lineGameObject.material.GetTextureOffset("_MainTex")), 1f));
