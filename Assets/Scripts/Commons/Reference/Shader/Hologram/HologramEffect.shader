Shader "SaberShad/HologramEffect"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // 颜色故障效果
                float color_glicth_noise = SampleNoiseMap(float2(_Time.x * _HologramColorGlitchData.x,
                                                                 _Time.x * _HologramColorGlitchData.y));
                ...
                float grain_noise = SampleNoiseMap(
                    (i.posWorld.xy * _HologramGrainData.xy + _Time.y * _HologramGrainSpeed));
                float grain_amount = lerp(_HologramGrainData.z, _HologramGrainData.w, grain_noise) *
                    _HologramGrainAffect;
